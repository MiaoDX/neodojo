...
| 14 | [mvp-real-conversion-gate.md](mvp-real-conversion-gate.md) | local prep/materialization/gpu-handoff/validation/import-demo ready; later GPU gate | Produce the first real GVHMR artifact for a short local Baduanjin clip on a GPU-capable machine. | Local prep writes source/trim metadata, source materialization can prepare the trimmed input, `make gpu-handoff` can package the external GPU handoff, and `make demo-real` can validate/import a returned export; final stop condition still requires a real GVHMR artifact from a GPU run. |
