...
archive 写出 GPU operator request manifest 和 README。
...
gvhmr-run-request-smoke` 会在 `make verify` 中覆盖 metadata-only request path；
`make gvhmr-colab-notebook-smoke` 会覆盖 generated notebook path。
