...
production teaching UI，或 end-to-end 的真实 generated motion artifact。
...
overlays、camera-preset metadata，以及显式的 SMPL-X scoring/G1 visual labels；
它还不是最终 teaching UI。
...
playback、Viser runtime preview、public-demo artifact，并运行 smoke check。
...
- [ ] 超出第一版可选 local runtime 和 generated preview screenshots 的 production
      Viser teaching UX
