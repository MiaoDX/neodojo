...
capture bundle manifest，以及最小 lint/build/quality-check 命令，并有可选
browser-rendered public-demo screenshot capture。它也可以为后续 GPU GVHMR run 写出 dry-run 或
...
它会清楚拒绝并要求 mesh-ready SMPL-X pose/shape parameters。当 motion record
包含 imported `smplx_parameters` 时，这个 gate 会先验证 parameter data 和 asset
descriptor，然后停在尚未实现的 renderer boundary。
...
headless Chromium public-demo screenshot manifest；直接 roboharness 与 simulator
video recording 仍是 follow-on work。
...
- [x] 可选 imported SMPL-X pose/shape parameter contract，用于未来 licensed mesh generation
...
- [ ] 超出 generated bundle 与 browser-rendered public-demo screenshot 的真实
      roboharness 或 simulator 离屏录制
...
- 🤖 [roboharness](https://github.com/MiaoDX/roboharness) — 给机器人仿真 agent 装上"眼睛"。neodojo 目前通过 generated capture bundle 和 browser-rendered public-demo screenshot 对齐它的多相机 evidence pattern；直接 simulator recorder 集成仍是 follow-on work
