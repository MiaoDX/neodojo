...
metadata-only GPU input bundle/archive smoke lanes、GPU execution probe、
metadata-only GPU run-request、Colab notebook、operator package、
operator-package archive 和 acquisition-status smoke lanes、fixture-only
real-artifact intake smoke lane、real-conversion completion audit。
...
execution probe、real-artifact intake smoke artifacts，上传默认
real-conversion audit 与 opt-in GitHub-route audit artifacts，以及
public-demo、browser-capture 与 capture-bundle artifacts，并在
repository variable `NEODOJO_DEPLOY_PAGES=true` 时把 fixture-only public demo
发布到 GitHub Pages。
...
request 和 notebook 放进一个 ignored operator package 目录并验证该 package，方便转交。
...
`make real-gvhmr-acquisition-status GVHMR_OPERATOR_PACKAGE_ARCHIVE=...` 会写出
non-failing acquisition status manifest：验证 operator archive，记录它是否已经可以交给
外部 GPU operator，嵌入现有 real-conversion audit status，并重复要求 returned
artifact 必须是 non-fixture 的合同。smoke variant 已接入 `make verify`，且不会上传
media。
...
- [x] 一命令生成单文件 operator package archive 的
      `make real-gpu-operator-package-archive`
