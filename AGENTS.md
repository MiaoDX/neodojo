...
- `PYTHONPATH=src python -m neodojo real-conversion prepare --id 03-006 --start 0 --end 12 --out outputs/real-conversion-gate` writes ignored source/trim metadata for the later GPU gate without downloading video or running GVHMR; `--local-video` records checksum and optional ffprobe metadata.
